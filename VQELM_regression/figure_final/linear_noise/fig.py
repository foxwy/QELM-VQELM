...
def Plot_data(flag=1):
    if flag == 1:
        wb = openpyxl.load_workbook('figure_final/linear_noise/results.xlsx')
    else:
    ...
    for k in range(1, 7):
        ...
        Te4 = []
        j = 100 * (k - 1) + 1
        for r1, r2, r3, r4, tr1, tr2, tr3, tr4 in zip(ws['D'][j:j + 10], ws['E'][j:j + 10], ws['F'][j:j + 10], ws['G'][j:j + 10], 
                                  ws['H'][j:j + 10], ws['J'][j:j + 10], 
                                  ws['L'][j:j + 10], ws['N'][j:j + 10]):
            R1.append(r1.value)
            R2.append(r2.value)
            R3.append(r3.value)
            R4.append(r4.value)
