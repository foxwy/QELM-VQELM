...
def Plot_data(flag=1):
    if flag == 1:
        wb = openpyxl.load_workbook('figure_final/xor/results.xlsx')
    else:
    ...
    for k in range(1, 5):
        ...
        Te4 = []
        j = 100 * (k - 1) + 1
        for r1, r2, r3, r4, tr1, tr2, tr3, tr4 in zip(ws['C'][j:j + 100], ws['D'][j:j + 100], ws['E'][j:j + 100], ws['F'][j:j + 100], 
                                  ws['G'][j:j + 100], ws['I'][j:j + 100], 
                                  ws['K'][j:j + 100], ws['M'][j:j + 100]):
            R1.append(r1.value)
            R2.append(r2.value)
            R3.append(r3.value)
            R4.append(r4.value)
        ...
        print('%s & %s & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.2e & %.2e & %.2e & %.2e \\\\' % (ws['A'][j].value, ws['B'][j].value,
    ...
    print('%.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.2e & %.2e & %.2e & %.2e \\\\' % (np.mean(r1_mean), np.mean(r2_mean), np.mean(r3_mean), np.mean(r4_mean),
