...
def Plot_data(flag=1):
    if flag == 1:
        wb = openpyxl.load_workbook('figure_final/breastcarcer_coimbra/results.xlsx')
    else:
    ...
    Tr4 = []
    j = 1
    for r1, r2, r3, r4, tr1, tr2, tr3, tr4 in zip(ws['A'][j:j + 100], ws['B'][j:j + 100], ws['C'][j:j + 100], ws['D'][j:j + 100], 
                              ws['E'][j:j + 100], ws['G'][j:j + 100], 
                              ws['I'][j:j + 100], ws['K'][j:j + 100]):
        R1.append(r1.value)
        R2.append(r2.value)
        R3.append(r3.value)
        R4.append(r4.value)
    ...
    print('\n%.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.3f & %.2e & %.2e & %.2e & %.2e \\\\' % (R1_mean, R2_mean, R3_mean, R4_mean,
